...
def all_btn_red():
	...
	Btn_Automatic.config(bg='#FF6D00', fg='#000000')
...
def all_btn_normal():
	...
	Btn_Automatic.config(bg=color_btn, fg=color_text)
...
def connection_thread():
	...
	while 1:
		...
		elif 'automatic' == car_info:
			funcMode = 1
			Btn_Automatic.config(bg='#FF6D00', fg='#000000')

		elif 'automaticOff' == car_info:
			funcMode = 0
			Btn_Automatic.config(bg=color_btn, fg=color_text)
...
def loop():					  #GUI
	global tcpClicSock,root,E1,connect,l_ip_4,l_ip_5,color_btn,color_text,Btn14,CPU_TEP_lab,CPU_USE_lab,RAM_lab,canvas_ultra,color_text,var_R,var_B,var_G,Btn_Steady,Btn_FindColor,Btn_WatchDog,Btn_Police,Btn_Automatic,Btn_Switch_1,Btn_Switch_2,Btn_Switch_3,Btn_Smooth   #The value of tcpClicSock changes in the function loop(),would also changes in global so the other functions could use it.
	while True:
		...
		root.title('Adeept DarkPaw Metal')	  #Main window title
		...
		Btn_Automatic = tk.Button(root, width=10, text='Automatic',fg=color_text,bg=color_btn,relief='ridge')
		Btn_Automatic.place(x=370,y=445)
		root.bind('<KeyPress-z>', call_Automatic)
		Btn_Automatic.bind('<ButtonPress-1>', call_Automatic)
